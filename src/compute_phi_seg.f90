...
subroutine compute_phi_seg(iseg, chainlen, coeff_ns, ns, nx, rx, q1_final, qmatrixA_final, chain_type)
!----------------------------------------------------------------------------------------------------------!
implicit none
!----------------------------------------------------------------------------------------------------------!
...
real(8), intent(in), dimension(0:nx,0:ns) :: q1_final, qmatrixA_final
...
if (iseg.ne.-1) then
    ...
    do kk = 0, nx
        phi_seg(kk) = (q1_final(kk,ns-iseg) * qmatrixA_final(kk,iseg)) / chainlen
    enddo
...
if (iseg.eq.-1) then
    ...
    do kk = 0, nx
        ...
        do ii = 0, ns
           write(122,'(2X,E16.9)',advance='no') (q1_final(kk,ii) * qmatrixA_final(kk,ns-ii)) / chainlen
        enddo
        write(122,*)
    enddo
endif
