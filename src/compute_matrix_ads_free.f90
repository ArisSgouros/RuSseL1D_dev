...
call convolution(chainlen_matrix, nx, ns_matrix, coeff, qfree_final, qfree_final, phi_free)
call convolution(chainlen_matrix, nx, ns_matrix, coeff, qads_final, qads_final, phi_loop)
call convolution(chainlen_matrix, nx, ns_matrix, coeff, qfree_final, qads_final, phi_tail)

do kk = 0, nx
    phi_ads(kk) = phi_matrix(kk) - phi_free(kk)
enddo

open (unit=37, file='o.ads_free_loop_tail.out.txt')
